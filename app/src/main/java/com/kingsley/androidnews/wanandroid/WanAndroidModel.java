...
public class WanAndroidModel implements WanAndroidContract.Model {

    private OnDataAcceptListener<List<GankIoAllDate.Results>> mOnDataAcceptListener;

    private WanAndroidModel(OnDataAcceptListener<List<GankIoAllDate.Results>> onDataAcceptListener){
        mOnDataAcceptListener = onDataAcceptListener;
    }

    @NonNull
    public static WanAndroidModel newInstance(OnDataAcceptListener<List<GankIoAllDate.Results>> onDataAcceptListener) {
        return new WanAndroidModel(onDataAcceptListener);
    }

    public String[] getTabs() {
        return new String[]{"每日推荐", "Android","iOS","休息视频","拓展资源", "前端","福利"};
    }
